...
Objective : This program uses the Directed Acyclic Graph (DAG) system to perform regular data updates. 
The process is managed by Apache Airflow, a tool used by data engineers. 
The purpose of the system is to transform and update the data efficiently.
...
def query_postgresql():
    ...
    df.to_csv('PROJECT_SIGMIFY_data_raw.csv')
    print("-------Data Saved------")
...
def clean_data():
    ...
    '''
    df = pd.read_csv('PROJECT_SIGMIFY_data_raw.csv')
    ...
    df.to_csv('./dags/PROJECT_SIGMIFY_data_clean.csv')
...
def insert_elasticsearch():
    ...
    df = pd.read_csv('./dags/SIGMIFY_PROJECT_clean.csv')
...
with DAG(
    ...
    schedule_interval='* * * * FRI',  # every friday at midnight
    catchup=False
) as dag:
