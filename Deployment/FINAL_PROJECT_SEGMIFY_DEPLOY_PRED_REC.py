...
def run():
    with st.form('Namanya apa'):
        st.title('Clustering Customer based on Customer Segmentation')

        InvoiceNo = st.slider('Invoice Number', min_value=0, max_value=50000, value=500, help='Enter your invoice number')
        StockCode = st.text_input('Stock Code', value='5000', help='Enter your stock code')
        Desc = st.text_input("Item Description:", value = 'white hanging heart t-light holder',help='Enter your item description').upper()
        Quantity = st.number_input('How Many Item?', min_value=0, max_value=8000000, value=2000000, help='Enter your item quantity')
        
        # Define the minimum date value
        min_date_value = date(2010, 1, 1)
        InvoiceDate = st.date_input("Invoice Date", value='default_value_today',format="MM.DD.YYYY",min_value=min_date_value,  help = 'Enter your Invoice Date')
        st.write("Your invoice date:", InvoiceDate)

        UnitPrice = st.number_input('Your Item Price?', min_value=0, max_value=500, value=20, help='Enter your item price')
        
        ID = st.slider('Your Customer ID', min_value=0, max_value=30000, value=18000, help='Enter your customer ID')
       ...
       'European Community', 'Malta', 'RSA'),help='Choose the country you currently live in')
        ...
        if submit:
            ...
            if submit:
                ...
                st.subheader("Prediction your customer level!")
                if pred == 0:
                    pred = 'Platinum'
                    st.write(f'Top 3 for customer level {pred} : {get_top_3_categories("Platinum")}')
                elif pred == 1:
                    pred = 'Diamond'
                    st.write(f'Top 3 for customer level {pred} : {get_top_3_categories("Diamond")}')
                elif pred == 2:
                    pred = 'Gold'
                    st.write(f'Top 3 for customer level {pred} : {get_top_3_categories("Gold")}')
                elif pred == 3:
                    pred = 'Silver'
                    st.write(f'Top 3 for customer level {pred} : {get_top_3_categories("Silver")}')
                else:
